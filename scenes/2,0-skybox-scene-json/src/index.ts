...
executeTask(async () => {
  ...
  if (signText) {
    ...
    if (sceneJson) {
      if (sceneJson.worldConfiguration && sceneJson.worldConfiguration.skyboxConfig && sceneJson.worldConfiguration.skyboxConfig.fixedTime) {
...
export function setupScene() {
  // Create a flat platform cube (16x0.2x16) at position 0,0.05,0
  ...
  Material.setPbrMaterial(platform, {
    albedoColor: Color4.fromHexString("#32CD32FF"), // Lime green with full alpha
    roughness: 0.8,
    metallic: 0.1
  })
