# Billboard Target Entity Test

Test scene for the `Billboard` component's `targetEntity` property.

## What this scene demonstrates

The scene is laid out as five labeled stations at parcel `1,1`, each with a `TextShape` sign describing what it shows:

- **A — No targetEntity**: a plain billboard with no `targetEntity` set. It faces the camera, same as before this feature existed.
- **B — Target a moving entity**: a billboard whose `targetEntity` points at a sphere orbiting nearby. The billboard visibly tracks the sphere instead of the camera.
- **C — Target that disappears/respawns**: a billboard whose `targetEntity` points at an entity that is deleted and re-created every few seconds. While the target is absent, the billboard freezes at its last orientation (reorientation is disabled); it resumes tracking as soon as the target exists again. A status label shows whether the target is currently `ACTIVE` or `ABSENT`.
- **D — billboardMode comparison**: two billboards target the same spinning/orbiting reference entity — one with `billboardMode: BM_Y` (yaw only, ignores the target's roll) and one with `billboardMode: BM_ALL` (also follows the target's Z roll). Comparing the two shows how `billboardMode` axes interact with `targetEntity`.
- **E — targetEntity = engine.CameraEntity**: a billboard whose `targetEntity` is explicitly set to the reserved camera entity (`engine.CameraEntity`, id 2). It behaves identically to case A (unset), demonstrating the documented equivalence.
